...
int output_tick(int state) {
	...
	switch (state) {
		...
		case output_display:
			if (cleartrigger) {
				LCD_Cursor(pos_tracker);
				LCD_WriteData(input);
				pos_tracker++;
				if (pos_tracker > 16) pos_tracker = 0;
			}
			cleartrigger = 0;
			break;
		default:
...
// challenge stuff reeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee
//char str_display[] = {'C','o','n','g','r','a','t','u','l','a','t','i','o','n','s','!'};

//enum position_tracker_SM {pt_start, pt_pos} position_tracker;

//int pt_tick(int state) {
	//switch (state) {
		//case pt_start:
			//LCD_DisplayString(1, "Congratulations!");
			//state = pt_pos;
...
const unsigned long int period = 15;
...
void main() {
	...
	//output_task.state = pt_start;
	//output_task.period = 1;
	//output_task.elapsedTime = 1;
	//output_task.TickFct = &pt_tick;
