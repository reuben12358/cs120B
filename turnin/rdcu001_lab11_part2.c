...
unsigned char phraselength = 40;
unsigned char screencharlength = 15;
unsigned char outputstring[] = " CS120B is Legend... wait for it DARY!  ";
enum display {start, next} displaySM;
...
int displayTick (int state) {
	...
	static unsigned int outputindex = 0;
	...
	switch(state) {
		case start:
			...
			break;
		case next:
			stringindex = outputindex;
			unsigned int i;
			for (i = 0; i < screencharlength; ++i) {
				LCD_Cursor(i);
				if (stringindex < phraselength) {
					LCD_WriteData(outputstring[stringindex]);
				}
				else {
					stringindex = 0;
				}
			...
			if (outputindex < phraselength) outputindex++;
			else outputindex = 0;
			break;
		default:
			break;
...
const unsigned long GCDperiod = 500;
...
void main() {
	...
	while (1) {
		...
		// }
		while(!TimerFlag);
		displayTask.state = displayTask.TickFct(displayTask.state);
		TimerFlag = 0;
	}
